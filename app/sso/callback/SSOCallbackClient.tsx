  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Store session in localStorage via useAuth hook
        login(data.user);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-zinc-50 px-4">
        <div className="w-full max-w-sm rounded-xl border border-[var(--color-border)] bg-white p-8 text-center shadow-lg">
          ...
          <a
            href={hubUrl}
            className="inline-block rounded-lg bg-[var(--color-primary)] px-6 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-[var(--color-primary-dark)]"
          >
            Back to iRam Hub
          </a>
        </div>
